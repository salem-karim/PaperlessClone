...
class GenAIMessageHandler(AbstractMessageHandler):
    """Handles GenAI summarization messages"""

    def __init__(self, rabbitmq_client, minio_client, genai_service: GenAIService, config: GenAIConfig):
        super().__init__(rabbitmq_client, minio_client, config)
        self.genai_service = genai_service
        self.config: GenAIConfig = config  # Type hint for IDE
    ...
    def _process_message(self, message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        request: GenAIRequestDto = message  # type: ignore
        ...
        try:
            ...
            ocr_text_object_key = request.get("ocr_text_object_key")
            ...
            if ocr_text:
                ...
                logger.info(f"Downloading OCR text for document {doc_id} from MinIO: {ocr_text_object_key}")
                ocr_text_bytes = self.minio.download_file(
                    self.minio.ocr_results_bucket,
                    ocr_text_object_key
                )
            ...
            logger.info(f"Generating summary for document {doc_id} using {self.config.GEMINI_MODEL}")
